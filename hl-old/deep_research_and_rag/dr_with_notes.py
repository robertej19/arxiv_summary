...
# =============================================================================
# CLI Progress / Trace Logging
# =============================================================================
DEFAULT_TRACE_FILE = "logs/dr_trace.jsonl"
SAFE_LOG_MAX = 1200  # console truncation

def _safe(s: str | None, n: int = SAFE_LOG_MAX) -> str:
    s = (s or "").replace("\u0000", "")
    return s if len(s) <= n else s[:n] + " …[truncated]"

class Spinner:
    def __init__(self, label="working", enabled=True):
        self.label = label
...
class Tracer:
    def __init__(self, enabled: bool, to_console: bool, show_prompts: bool, path: str):
        self.enabled = enabled
    ...
    def event(self, kind: str, data: Dict[str, Any]):
        ...
        # console (pretty/truncated)
        if self.to_console:
            pretty = dict(payload)
            if not self.show_prompts:
                # redact large fields on console; keep them in the file
                for k in ("prompt", "response", "snippets", "evidence_blobs", "citation_list", "draft"):
                    if k in pretty:
                        pretty[k] = _safe(str(pretty[k]))
            print(f"◦ {kind}: {json.dumps(pretty, ensure_ascii=False)}")
            sys.stdout.flush()
...
def _find_span(hay: str, needle: str) -> Optional[Tuple[int, int]]:
    ...
    if i < 0:
        ...
        return None if norm(hay or "").find(norm(needle or "")) < 0 else None
...
# =============================================================================
# Ask wrapper (records every prompt/response)
# =============================================================================
def ask(model: LlamaCppModel, tracer: Tracer, label: str, prompt: str) -> str:
    tracer.event("llm_prompt", {"label": label, "prompt": prompt})
    sp = Spinner(label, enabled=tracer.to_console); sp.start()
    out = model.generate([{"role": "user", "content": prompt}])
    sp.stop()
    resp = getattr(out, "content", str(out))
    tracer.event("llm_response", {"label": label, "response": resp})
    return resp

# =============================================================================
# Pipeline stages
# =============================================================================
def plan_queries(model: LlamaCppModel, tracer: Tracer, question: str) -> List[str]:
    out = ask(model, tracer, "plan_queries", PLANNER_PROMPT.format(question=question))
    arr = _extract_json_array(out)[:MAX_QUERIES]
    tracer.event("plan_queries_parsed", {"queries": arr or [question]})
    return arr or [question]

def extract_claims(model: LlamaCppModel, tracer: Tracer, question: str) -> List[str]:
    out = ask(model, tracer, "extract_claims", CLAIMS_PROMPT.format(question=question))
    ...
    claims = claims[:8]
    tracer.event("claims_parsed", {"claims": claims})
    return claims

def rerank_hits_with_llm(model: LlamaCppModel, tracer: Tracer, query: str, hits: List[Dict[str, Any]], top_n: int) -> List[Dict[str, Any]]:
    # build small snippet list
    lines = []
    for i, h in enumerate(hits[:10], 1):
        snip = _safe(h.get("snippet", ""), 280)
        lines.append(f"{i}. ({h.get('doc_id')}::{h.get('chunk_id')}) {snip}")
    prompt = RERANK_PROMPT.format(query=query, snippets="\n".join(lines))
    resp = ask(model, tracer, "rerank_hits", prompt)
    try:
    ...
    tracer.event("rerank_result", {"kept": [(h.get("doc_id"), h.get("chunk_id")) for h in kept], "scores": scores[:len(kept)]})
    return kept

def read_and_quote(tracer: Tracer, read_tool: ReadChunkTool, hit: Dict[str, Any], keywords: List[str]) -> List[Evidence]:
    tracer.event("read_start", {"doc_id": hit.get("doc_id"), "chunk_id": hit.get("chunk_id")})
    ...
    tracer.event("read_end", {
        "doc_id": hit.get("doc_id"),
        "chunk_id": hit.get("chunk_id"),
        "quotes": [q.quote for q in evidences]
    })
    return evidences

def llm_contradiction_check(model: LlamaCppModel, tracer: Tracer, qa: str, qb: str) -> bool:
    resp = ask(model, tracer, "contradiction_check", CONTRADICT_PROMPT.format(qa=_safe(qa, 400), qb=_safe(qb, 400)))
...
def contradictions_for_claim(model: LlamaCppModel, tracer: Tracer, evs: List[Evidence]) -> List[str]:
    issues = []
    for i in range(len(evs)):
        for j in range(i + 1, len(evs)):
            a, b = evs[i], evs[j]
            # quick heuristic: numeric mismatch
            nums1 = re.findall(r"-?\d+(?:\.\d+)?", a.quote or "")
            nums2 = re.findall(r"-?\d+(?:\.\d+)?", b.quote or "")
            if nums1 and nums2 and set(nums1) != set(nums2):
                if llm_contradiction_check(model, tracer, a.quote, b.quote):
...
def write_answer(model: LlamaCppModel, tracer: Tracer, question: str, claims: List[str],
    ...
    tracer.event("writer_prompt", {"claims": claims, "evidence_blobs": evidence_blobs, "citation_list": citation_list})
    draft = ask(model, tracer, "writer_draft", prompt)
    return draft

def critic_pass(model: LlamaCppModel, tracer: Tracer, draft: str, claims: List[str],
    ...
    prompt = CRITIC_PROMPT.format(
        draft=_safe(draft, 6500),
        claims_bullets=claims_bullets,
        evidence_map="\n".join(evidence_map_lines),
    )
    resp = ask(model, tracer, "critic", prompt)
    try:
    ...
    tracer.event("critic_result", obj)
    return obj

# =============================================================================
# Main pipeline
# =============================================================================
def deep_research(question: str, verbose: bool, trace: Tracer) -> str:
    log = Logger(verbose)
    ...
    # PLAN & CLAIMS
    queries = plan_queries(model, trace, question)
    claims = extract_claims(model, trace, question)
    ...
    trace.event("search_hits", {"count": len(raw_hits)})
    log.info("Raw hits (unique): %d", len(raw_hits))

    # RERANK
    reranked = rerank_hits_with_llm(model, trace, query=question, hits=raw_hits, top_n=TOP_AFTER_RERANK)
    log.info("Reranked top: %d", len(reranked))

    # READ
    pad = Notepad()
    for hit in reranked[:MAX_READ_CHUNKS]:
        for ev in read_and_quote(trace, read_tool, hit, q_keywords):
            pad.add(ev)
    trace.event("evidence_collected", {"items": len(pad.notes), "sources": pad.unique_source_count()})
    log.info("Evidence: %d item(s) from %d source(s)", len(pad.notes), pad.unique_source_count())
    ...
    if pad.unique_source_count() < MIN_UNIQUE_SOURCES and rounds_left > 0:
        ...
        for it in items:
            ...
            if any((n.doc_id == it["doc_id"] and n.chunk_id == it["chunk_id"]) for n in pad.notes):
                continue
            for ev in read_and_quote(trace, read_tool, it, q_keywords):
                pad.add(ev)
        trace.event("saturation_done", {"items": len(pad.notes), "sources": pad.unique_source_count()})
        log.info("After saturation: %d source(s), %d evidence", pad.unique_source_count(), len(pad.notes))

    if pad.unique_source_count() < MIN_UNIQUE_SOURCES:
        trace.event("abort_insufficient_sources", {"sources": pad.unique_source_count()})
    ...
    for i, evs in claim_map.items():
        if len(evs) >= 2:
            issues = contradictions_for_claim(model, trace, evs[:3])
            if issues:
                contradictions.append((i, issues))
    trace.event("contradictions", {"count": len(contradictions), "details": contradictions})

    # WRITE
    evidence_blobs, citation_list, idx_of_doc, focus = build_evidence_pack(pad, claims, q_keywords)
    draft = write_answer(model, trace, question, claims, evidence_blobs, citation_list)

    # CRITIC
    critic = critic_pass(model, trace, draft, claims, claim_map, idx_of_doc)
...
def main():
    ...
    ap.add_argument("question", nargs="*", help="Research question")
    ap.add_argument("--quiet", "-q", action="store_true", help="Silence progress logs")
    ap.add_argument("--trace", action="store_true", help="Record prompts & responses to JSONL and console")
    ap.add_argument("--trace-file", default=DEFAULT_TRACE_FILE, help="Where to write JSONL traces")
    ap.add_argument("--show-prompts", action="store_true", help="Also print full prompts to console (file always gets full)")
    args = ap.parse_args()

    verbose = not args.quiet
    tracer = Tracer(enabled=args.trace, to_console=args.trace, show_prompts=args.show_prompts, path=args.trace_file)

    question = " ".join(args.question) if args.question else \
        "Summarize why PPO is more practical than TRPO. Provide at least 2 citations."

    print("╭──────────────────────────────── Deep Research ────────────────────────────────╮")
    print("│ Question:", question)
    print("╰──────────────────────────────────────────────────────────────────────────────╯")

    answer = deep_research(question, verbose=verbose, trace=tracer)
    print(answer)
